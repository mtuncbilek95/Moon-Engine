...
namespace Moon
{
	...
	XMINT2 TextureObject::GetSpriteCount()
	{
		return m_AtlasSpriteCount;
	}
	...
	bool TextureObject::CreateTexture()
	{
		...
		HRESULT hr = DXManager::GetInstance().GetDevice()->CreateTexture2D(&m_TextureBufferDesc, &TextureResourceData, &m_TextureBuffer);
	...
	bool TextureObject::CreateShaderResourceView()
	{
		...
		HRESULT hr = DXManager::GetInstance().GetDevice()->CreateShaderResourceView(m_TextureBuffer.Get(), &m_ShaderResourceDesc, &m_ShaderResourceView);
		if (FAILED(hr))
		{
	...
	bool TextureObject::CreateSamplerState()
	{
		...
		HRESULT hr = DXManager::GetInstance().GetDevice()->CreateSamplerState(&SamplerDesc, &m_SamplerState);
	...
	bool TextureObject::CreateBlendState()
	{
		...
		HRESULT hr = DXManager::GetInstance().GetDevice()->CreateBlendState(&blendDesc, &m_BlendState);
